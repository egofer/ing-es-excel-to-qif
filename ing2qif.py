...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Convierte extracto Excel ING a QIF", epilog="Ejemplo: python xls_to_qif.py extracto.xlsx -o salida.qif -v")
    parser.add_argument("excel_file", help="Ruta al archivo Excel.")
    parser.add_argument("-o", "--output", help="Ruta QIF salida (defecto: nombre.qif).")
    parser.add_argument("--encoding", default="utf-8", choices=["utf-8", "cp1252", "iso-8859-1"], help="Codificación salida.")
...
def parse_spanish_decimal(decimal_val, row_num, verbose=False):
    ...
    try:
        ...
        if verbose:
            print(f"  [DEBUG] Fila {row_num}: No se pudo convertir importe '{decimal_str}'.")
        return None
...
def find_header_and_metadata(excel_filepath, expected_header, verbose=False):
    ...
    if verbose: print("Buscando cabecera y metadatos...")
    header_row_index = -1
    account_info = {}
    try:
        # Leemos las primeras filas sin cabecera para buscar la estructura
        df_pre = pd.read_excel(excel_filepath, header=None, keep_default_na=False, nrows=20)
    ...
    for idx, row_values in enumerate(df_pre.values.tolist()):
        ...
        # CORRECCIÓN: Manejo robusto de columnas vacías
        current_signature = row_str[:len(expected_header)]
        
        if verbose and not header_found_flag:
            # Debug para ver qué está leyendo si falla
            # print(f"  Check fila {idx}: {current_signature}")
            pass
        ...
        if not header_found_flag and current_signature == expected_header:
            ...
            if verbose:
                print(f"Cabecera detectada en índice {header_row_index} (Fila Excel {header_row_index + 1}).")
...
def read_excel_data(excel_filepath, header_row_index, verbose=False):
    ...
    try:
        ...
        df_data = pd.read_excel(excel_filepath, header=header_row_index, keep_default_na=False)
        df_data.columns = df_data.columns.map(lambda x: x.strip() if isinstance(x, str) else x)
        return df_data
    except Exception as e:
...
def get_excel_date(raw_val):
    ...
    try:
        ...
        if isinstance(raw_val, (int, float)):
            # Excel considera 1900 un año bisiesto (bug histórico), pero para fechas actuales 
            # usar base 1899-12-30 suele funcionar en Pandas/Python
...
def process_transaction_row(row, original_excel_row, col_map, prefix_pattern, all_caps_pattern, verbose=False):
    ...
    try:
        ...
        if tx_date is None:
            if verbose: print(f"  OMITIENDO fila {original_excel_row}: Fecha inválida '{raw_date_val}'.")
            return None
            
        if not (REASONABLE_START_DATE <= tx_date <= REASONABLE_END_DATE):
            if verbose: print(f"  AVISO: Fila {original_excel_row}: Fecha '{tx_date}' fuera rango razonable.")

        # --- Importe ---
        raw_amount_val = row.get(col_map['amount'])
        amount = parse_spanish_decimal(raw_amount_val, original_excel_row, verbose)
        if amount is None:
            return None
        ...
        # --- Lógica memo ---
        memo_text = extract_memo_text(description, prefix_pattern, all_caps_pattern, verbose)

        # --- Construcción categoría ---
        category_parts = [part for part in [category_csv, subcategory_csv] if part]
    ...
    try:
        with open(qif_filepath, mode='w', encoding=output_encoding, errors='replace') as outfile:
            ...
            for tx in transactions:
                ...
                if tx['category']: outfile.write(f"L{tx['category']}\n")
                if tx['memo']: outfile.write(f"M{tx['memo']}\n")
                outfile.write("^\n")
        return True
    except Exception as e:
...
def main():
    ...
    if args.output:
        ...
        output_filename = re.sub(r'\.[Xx][Ll][Ss][Xx]?$', '', base_name, flags=re.IGNORECASE) + ".qif"

    header_idx, _ = find_header_and_metadata(args.excel_file, EXPECTED_HEADER, args.verbose)
    if header_idx == -1: sys.exit(1)

    df_data = read_excel_data(args.excel_file, header_idx, args.verbose)
    if df_data is None: sys.exit(1)
    ...
    success = generate_qif_file(processed_transactions, output_filename, args.encoding, args.verbose)
